...
/*
 *
 * This class is called during form processing to setup our PayPal Subscriptions checkout using the NF_Paypal_Process class.
 *
 * @since 1.0
 *
 */
...
class NF_Paypal_Form_Process
{
	...
	public function __construct() {
		...
		// Bail if this form isn't setup to use PayPal Subscriptions.
		if ( $ninja_forms_processing->get_form_setting( 'paypal_subscriptions' ) != 1 )
    		return;
	...
	function do_checkout() {
		...
		if ( ( isset ( $plugin_settings['debug'] ) and $plugin_settings['debug'] == 1 ) or ( ! isset ( $plugin_settings['debug'] ) and NINJA_FORMS_paypal_subscriptions_DEBUG ) ) {
			...
			echo '<h3>';
	    	echo __( 'The debugging information above and any error output below has been generated by Ninja Forms - PayPal Subscriptions. To remove this information and any errors, please disable debugging in the Forms->PayPal Settings menu.', 'ninja-forms-paypal-subscriptions' );
	    	echo '</h3>';
	...
	public function get_product_info() {
		...
		if ( is_array ( $total ) ) { // If it is, then get the items from it.
						...
						if ( $field['type'] == '_list' ) {
							...
							if ( is_array ( $user_value ) ) {
							  	foreach ( $user_value as $val ) {
									...
									$item['L_PAYMENTREQUEST_0_NAME'.$x] = apply_filters( 'ninja_forms_paypal_subscriptions_product_name', $field_label, $field_id );
									$item['L_PAYMENTREQUEST_0_DESC'.$x] = apply_filters( 'ninja_forms_paypal_subscriptions_product_desc', $field_desc, $field_id );
			...
			if ( empty( $item ) ) {
				$product_name = __( 'Total1', 'ninja-forms-paypal-subscriptions' );
				$product_desc = '';
				$item = array(
				   'L_PAYMENTREQUEST_0_NAME0' => apply_filters( 'ninja_forms_paypal_subscriptions_product_name', $product_name, false ),
				   'L_PAYMENTREQUEST_0_DESC0' => apply_filters( 'ninja_forms_paypal_subscriptions_product_desc', $product_desc, false ),
...
/*
 *
 * Function that initialized our PayPal Subscriptions processing
 *
 * @since 1.0
 * @return void
 */

function ninja_forms_paypal_subscriptions_process(){
	$NF_Paypal_Form_Process = new NF_Paypal_Form_Process();
}

add_action( 'ninja_forms_process', 'ninja_forms_paypal_subscriptions_process', 9999 );