...
class NF_Paypal_Subs_Deprecated
{
	...
	/*
	 *
	 * Function that modifies our view subs table header if the form has PayPal Subscriptions enabled.
	 *
	 * @since 1.0
	 * @return void
	 */
	...
	function modify_header( $form_id ) {
		...
		if ( isset ( $form['data']['paypal_subscriptions'] ) AND $form['data']['paypal_subscriptions'] == 1 ) {
			?>
			<th><?php _e( 'PayPal Status', 'ninja-forms-paypal-subscriptions' );?></th>
			<th><?php _e( 'Transaction ID', 'ninja-forms-paypal-subscriptions' );?></th>
			<?php			
		}
	...
	function filter_csv_labels( $label_array, $sub_id_array ) {
		...
		if ( isset ( $form['data']['paypal_subscriptions'] ) AND $form['data']['paypal_subscriptions'] == 1 ) {
			array_splice($label_array[0], 2, 0, __( 'PayPal Status', 'ninja-forms-paypal-subscriptions' ) );
			array_splice($label_array[0], 3, 0, __( 'Transaction ID', 'ninja-forms-paypal-subscriptions' ) );			
		}
		return $label_array;	
	...
	/*
	 *
	 * Function that modifies each row of our CSV by adding PayPal Status and Transaction ID if the form is set to use PayPal Subscriptions.
	 *
	 * @since 1.0
	...
	function change_paypal_status() {
		...
		if( $sub_id != '' ){
			...
			if ( isset( $form['data']['paypal_subscriptions'] ) AND $form['data']['paypal_subscriptions'] == 1 ) {
				...
				?>
				<div>
					<?php _e( 'PayPal Status', 'ninja-forms-paypal-subscriptions' ); ?>	
				...
				<div>
					<?php _e( 'PayPal Transaction ID', 'ninja-forms-paypal-subscriptions' );?>: 
